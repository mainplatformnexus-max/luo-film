...
const Watch = () => {
  ...
  useEffect(() => {
    ...
    if (firebaseState?.firebaseId) {
      ...
      getEpisodesBySeries(firebaseState.firebaseId).then(setEpisodes).catch(() => {});
    }
    ...
    return () => { unsub1(); };
  }, [firebaseState?.firebaseId, isSport]);
  ...
  const actorList = drama.actors ? drama.actors.split(",").map(a => a.trim()).filter(Boolean) : [];
  ...
  const handleDownload = () => {
    const downloadUrl = (drama as any).downloadLink || drama.streamLink;
    if (downloadUrl) {
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="relative w-full max-h-[480px] aspect-video bg-black">
            {requiresSubscription ? (
              ...
              <ArtPlayerComponent src={drama.streamLink} poster={drama.image} autoplay />
            ) : (
              <>
        ...
        <div className="w-full lg:w-[300px] border-l border-border flex-shrink-0">
          ...
          <div className="overflow-y-auto max-h-[400px] scrollbar-thin">
            {episodes.length > 0 ? episodes.map((ep, i) => (
              <div key={ep.id} className={`flex gap-3 p-3 cursor-pointer hover:bg-secondary/50 transition-colors ${i === 0 ? "bg-secondary/30" : ""}`}
                onClick={() => ep.streamLink && navigate(`/watch/${ep.id}`, { state: { firebaseId: ep.id, title: ep.name, streamLink: ep.streamLink, downloadLink: ep.downloadLink } })}>
                <div className="flex-1 min-w-0">
                  <p className={`text-[11px] font-medium line-clamp-2 ${i === 0 ? "text-primary" : "text-foreground"}`}>
                    EP{ep.episodeNumber} · {ep.name}
                  </p>
                  {ep.streamLink && <Play className="w-3 h-3 text-primary mt-1" />}
                </div>
              </div>
            )) : (
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <div className="w-6 h-6 bg-primary rounded flex items-center justify-center">
                  <span className="text-primary-foreground text-[8px] font-bold">LF</span>
                </div>
                <span className="text-muted-foreground text-xs">Download TV APP</span>
              </div>
              <button onClick={handleWatchOnTV} className="text-primary text-xs font-medium">Watch on TV ›››</button>
            </div>
          </div>
        </div>
      </div>
