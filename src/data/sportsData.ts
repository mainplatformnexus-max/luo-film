...
export interface ApiMatch {
  ...
  team1: ApiTeam;
  team2: ApiTeam;
  status: string; // "MatchLiving", "MatchNotStart", "MatchEnded"
  playType: string;
  playPath: string;
  startTime: string;
  endTime: string;
  type: string; // "football", "cricket", etc.
  timeDesc: string;
...
export function normalizeMatch(m: ApiMatch): SportMatch {
  ...
  const isLive = m.status === "MatchLiving" || m.statusLive === 1;
  ...
  return {
    ...
    homeScore: parseInt(m.team1.score, 10) || 0,
    awayScore: parseInt(m.team2.score, 10) || 0,
...
export function getTodayTimestamps() {
  ...
  const end = start + 86399999; // 24h - 1ms
...
export async function fetchMatchesV5(): Promise<SportMatch[]> {
  try {
    ...
    // v5 returns flat list of matches
...
export async function fetchMatchesV3(): Promise<SportMatch[]> {
  try {
    ...
    const json = await res.json();
    if (json.code !== 0) return [];
    // v3 has nested structure: data.list[].matchList[]
...
export async function fetchAllMatches(): Promise<SportMatch[]> {
  ...
  // Merge & dedupe by id, prefer v3 data (has more detail)
  const map = new Map<string, SportMatch>();
  for (const m of v5) map.set(m.id, m);
  for (const m of v3) map.set(m.id, m); // v3 overwrites
